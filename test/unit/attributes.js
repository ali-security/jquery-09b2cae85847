...
test( "jQuery.find.attr does not recurse on non-lowercase boolean attributes", function() {
	expect( 3 );
...
test( "non-lowercase boolean attribute selectors should not crash", function() {
	...
	jQuery.each( [ "[Checked]", "[CHECKED]", "[requiRed]", "[Checked][requiRed]" ],
		function( i, selector ) {
			try {
				strictEqual( elems.filter( selector ).length, 1,
					"'" + selector + "' matches only the boolean-flagged input" );
			} catch ( e ) {
				ok( false, "'" + selector + "' threw: " + e );
			}
		});
});
